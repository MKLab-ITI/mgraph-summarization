...
public class Item implements Serializable {
	...
	public Collection<NamedEntity> getNamedEntities(String type) {
		if(entities == null)
			return null;
		
		List<NamedEntity> typeEntites = new ArrayList<NamedEntity>();
	...
	public void incReposts(int reposts) {
		this.reposts += reposts;
	}
	...
	public static void fold(Map<String, Item> itemsMap, Collection<Collection<String>> clusters) {
		for(Collection<String> cluster : clusters) {
			...
			for(String id : cluster) {
				Item item = itemsMap.remove(id);
				if(item != null) {
					items++;
					totalReposts += item.getReposts();
					
					if(item.getHashtags() != null)
						allHashtags.addAll(item.getHashtags());
					
					if(item.getUrls() != null)
						allURLS.addAll(item.getUrls());
					
					if(item.getMediaItems() != null)
						media.putAll(item.getMediaItems());
					
					pTime += item.publicationTime;
				}
			}
			
			Item mergedItem = new Item();
			mergedItem.setId(clusterId);
			mergedItem.setReposts(totalReposts);
			mergedItem.setHashtags(new ArrayList<String>(allHashtags));
			mergedItem.setUrls(new ArrayList<String>(allURLS));
			mergedItem.setPublicationTime(pTime/items);
			mergedItem.setMediaUrls(media);
			
			itemsMap.put(clusterId, mergedItem);
		}
	}

...
public class LDA extends TopicDetector {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -3889334869743250843L;
	...
	private Map<Integer, Collection<String>>  associations;
	...
	public void run(Map<String, Vector> vectors, Map<String, Item> items)
		...
		InstanceList instances = getInstancesFromVectors(vectors, stopwords);
	...
	public void train(InstanceList instances) throws IOException {
		...
		_model.setNumIterations(iterations);
		_model.setTopicDisplay(iterations/10, 20);
	...
	public Map<Integer, Collection<String>> getTopicAssociations(InstanceList instances) {
		Map<Integer, Collection<String>> associations = new TreeMap<Integer, Collection<String>>();
	...
	public Map<Integer, List<String>> getTopicAssociations(InstanceList instances, int iterations) {
		...
		for(int i=0; i<iterations ; i++) {
			Map<Integer, Collection<String>> temp = getTopicAssociations(instances);
			for(Entry<Integer, Collection<String>> e : temp.entrySet()) {
				Integer topicId = e.getKey();
				Collection<String> newAssociations = e.getValue();
	...
	public static InstanceList getInstancesFromVectors(Map<String, Vector> vectorsMap, Collection<String> stopwords) throws IOException {
		List<Pipe> pipeList = getPipelist(stopwords);
		InstanceList instances = new InstanceList (new SerialPipes(pipeList));

		for(Entry<String, Vector> entry : vectorsMap.entrySet()) {
	...
	public static void main(String args[]) throws Exception {
		...
		LDA lda = new LDA(T);
		lda.run(vectorsMap, itemsMap);
		
		Map<Integer, Collection<String>> associations = lda.getTopicAssociations();
		...
		IOUtil.saveClusters(associations.values(), "/disk1_data/Datasets/" + dataset + "/lda-clusters.txt");
